...
Do not introduce a UIFlow client boundary at all when the route is naturally server-rendered and has no meaningful interactive journey. UIFlow qualification comes before client-boundary creation.
...
Straightforward server-side fetching followed by static/display rendering does not justify UIFlow by itself.
...
Do not create channels for ordinary parent/child React communication; prefer direct props/handlers unless both sides are independent flows that genuinely require coordination.
